...
export function Dashboard() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const [agentsResponse, tracesResponse, summaryResponse, executionsResponse] = await Promise.all([
        fetch('/api/agents'), fetch(`/api/traces${suffix}`), fetch(`/api/traces/summary${suffix}`), fetch('/api/executions?state=executing'),
      ]);
      if (![agentsResponse, tracesResponse, summaryResponse, executionsResponse].every((response) => response.ok)) throw new Error();
      ...
      setData({ traces: await tracesResponse.json(), summary: await summaryResponse.json() });
      setExecutions((await executionsResponse.json()).items as Execution[]);
    } catch {
      setNotice('Não foi possível atualizar os dados. Verifique a API local.');
    }
  }, [active?.name, status]);
  ...
  async function createAgent(event: FormEvent<HTMLFormElement>) {
    ...
    setCreating(false);
    if (!response.ok) return setNotice(payload.detail || 'Não foi possível cadastrar este agente.');
    event.currentTarget.reset();
    setSelected(payload.agent_id);
    setNotice('Agente criado. Rode a demo para gerar o primeiro trace.');
    await load();
  }
  ...
  async function demo() {
    if (!active) return;
    setRunning(true);
    setNotice('Executando cenário sintético e gravando spans…');
    const response = await fetch(`/api/agents/${active.agent_id}/demo`, { method: 'POST' });
    const payload = await response.json();
    setRunning(false);
    if (!response.ok) return setNotice(payload.detail || 'A demo não foi concluída.');
    setNotice(`Demo concluída: ${payload.span_count} spans persistidos para ${active.name}.`);
    await load();
  }

  const summary = data?.summary;
  return <div className="workbench">
    <aside className="agents-panel" aria-label="Agentes">
      <div className="panel-heading"><span>Agentes</span><span className="count">{agents.length}</span></div>
      <div className="agent-list">{agents.length ? agents.map((agent) => <button className={`agent-row ${agent.agent_id === selected ? 'selected' : ''}`} key={agent.agent_id} onClick={() => setSelected(agent.agent_id)}><span className="agent-mark">{agent.name.slice(0, 1).toUpperCase()}</span><span><strong>{agent.name}</strong><small>{agent.description || 'Sem descrição'}</small></span></button>) : <p className="empty-mini">Ainda não há agentes. Crie o primeiro para começar.</p>}</div>
      <form className="agent-form" onSubmit={createAgent}><label htmlFor="agent-name">Novo agente</label><input id="agent-name" name="name" placeholder="ex.: research-agent" maxLength={200} required /><input name="description" placeholder="Descrição opcional" maxLength={500} /><button className="button ghost" disabled={creating}>{creating ? 'Criando…' : 'Cadastrar agente'}</button></form>
    </aside>
    <section className="observatory">
      <div className="controlbar"><div><span className="overline">Ambiente local</span><h1>{active ? active.name : 'Observatório de agentes'}</h1><p>{active?.description || 'Crie um agente para começar uma sessão de observabilidade.'}</p></div><div className="controls"><select aria-label="Filtrar traces por status" value={status} onChange={(event) => setStatus(event.target.value)}><option value="">Todos os resultados</option><option value="success">Com sucesso</option><option value="error">Com erro</option></select><button className="button primary" onClick={demo} disabled={!active || running}>{running ? 'Executando demo…' : 'Rodar demo'}</button></div></div>
